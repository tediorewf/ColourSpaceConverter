...
public partial class RGBChannelsExtractionForm : Form {
  public RGBChannelsExtractionForm() { InitializeComponent(); }
  ...
  private void TrySetImage(string filename) {
    try {
      ...
      var histogramBitmap =
          CreateHistogram(baseBitmap, histogramPictureBox.Size);
  ...
  private static Bitmap[] ExctractChannels(Bitmap baseBitmap) {
    ...
    using (var fastBaseBitmap = new FastBitmap(baseBitmap)) using (
        var fastRedChannelBitmap = new FastBitmap(
            colourChannels
            [0])) using (var fastGreenChannelBitmap =
                             new FastBitmap(
                                 colourChannels
                                 [1])) using (var fastBlueChannelBitmap =
                                                  new FastBitmap(
                                                      colourChannels[2])) {
      for (int x = 0; x < baseBitmap.Width; x++) {
        for (int y = 0; y < baseBitmap.Height; y++) {
          var currentPixelColour = fastBaseBitmap[x, y];
          fastRedChannelBitmap[x, y] =
              Color.FromArgb(currentPixelColour.A, currentPixelColour.R, 0, 0);
          fastGreenChannelBitmap[x, y] =
              Color.FromArgb(currentPixelColour.A, 0, currentPixelColour.G, 0);
          fastBlueChannelBitmap[x, y] =
  ...
  private static Bitmap CreateHistogram(Bitmap baseBitmap, Size size) {
    ...
    using (var fastBaseBitmap = new FastBitmap(baseBitmap)) using (
        var fastHistogramBitmap = new FastBitmap(histogramBitmap)) {
      ...
      for (int x = 0; x < 256; x++) {
        ...
        int relativeY =
            ...
            redChannelFrequencies[x] * minY * histogramBitmap.Height / maxY;
        DrawVerticalLine(fastHistogramBitmap, relativeX, relativeY, Color.Red);

        relativeY = histogramBitmap.Height - greenChannelFrequencies[x] * minY *
                                                 histogramBitmap.Height / maxY;
        DrawVerticalLine(fastHistogramBitmap, relativeX, relativeY,
                         Color.Green);

        relativeY = histogramBitmap.Height - blueChannelFrequencies[x] * minY *
                                                 histogramBitmap.Height / maxY;
  ...
  private static void DrawVerticalLine(FastBitmap fastBitmap, int relativeX,
                                       int relativeY, Color colour) {
